<script setup>
  ...
  const {country, region, regions } = storeToRefs(regionStore)
  const {updateRegion} = regionStore
...
<template>
  <div class="relative text-left">
    <div v-if="region">
      ...
      >
        <span class="mr-1">{{ country.display_name }} / {{ region?.currency_code?.toUpperCase() }}</span>
        <svg
    ...
    <div
      v-if="regions.length > 0 && showRegionsMenu"
    ...
    >
      <ul class="border-b border-ui last:border-b-0">
        <li v-for="(region) in regions" :key="region.id" class="not-last:border-b border-gray-300">
          <button
            ...
            @click="updateRegion(region, country) ; (showRegionsMenu = false)"
          >
